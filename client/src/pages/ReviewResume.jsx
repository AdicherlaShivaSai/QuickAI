  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <button disabled={loading} className='w-full mt-6 bg-gradient-to-r from-[#00DA83] to-[#009B83] text-white text-sm px-4 py-2 rounded-lg cursor-pointer flex items-center justify-center gap-2'>
          {
            loading ? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin'></span> : <FileText className='w-6'/>
          }
          Review Resume
        </button>
      </form>
