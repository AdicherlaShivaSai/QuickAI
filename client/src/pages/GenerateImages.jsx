  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        placeholder='Describe the image you want to see' required/>

        <p className='mt-4 text-sm font-semibold'>Style</p>

        <div className='mt-3 flex flex-wrap gap-3 sm:max-w-9/11'>
          {ImageStyle.map((item)=>(
            <span onClick={()=>setSelectedStyle(item)} className={`text-xs px-4 py-1 rounded-full border cursor-pointer ${selectedStyle === item ? 'bg-green-50 text-green-700' : 'border-gray-300 text-gray-500'}`}
            key={item}>{item}</span>
          ))}
        </div>
        ...
        <br />
        <button disabled={loading} className='w-full mt-6 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white text-sm px-4 py-2 rounded-lg cursor-pointer flex items-center justify-center gap-2'>
          {
            loading ? <span className='w-4 h-4 my-1 rounded-full border-2 border-t-transparent animate-spin'></span> : <Image className='w-6'/>
          }
          Generate Image
        </button>
      </form>
